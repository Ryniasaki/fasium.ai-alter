...
from .runtime_cache import cache_get_json, cache_key, cache_set_json
...
def setup_metrics(app: FastAPI, service_name: str) -> None:
    ...
    async def metrics_endpoint():
        cache_id = cache_key("tenant-service", "metrics")
        cached_payload = cache_get_json(cache_id)
        if isinstance(cached_payload, dict) and isinstance(cached_payload.get("body"), str):
            return Response(content=cached_payload["body"].encode("utf-8"), media_type=CONTENT_TYPE_LATEST)

        body = generate_latest().decode("utf-8")
        cache_set_json(cache_id, {"body": body}, ttl_seconds=2)
