...
export default function ExtractStripePage() {
  ...
  const [stripeThicknessScale, setStripeThicknessScale] = useState(1)
  ...
  const drawStripesAtAngle = useCallback(
    (
      ...
      const angleRad = (angleOffsetDeg * Math.PI) / 180
      ctx.save()
  ...
  const handleSourceUpload = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        const palette = await extractApiClient.requestPlaidPalettes(file)
        ...
        if (stripePattern.length === 0) {
          setStripeUnits([])
          setUploadError("未检测到清晰的格子结构，请调整 prompt 以突出格子条纹，或尝试更换更清晰的拍摄角度。")
  ...
  const handleReextract = useCallback(async () => {
    ...
    try {
      ...
      const palette = await extractApiClient.requestPlaidPalettes(workingFile)
      const groups = Array.isArray(palette?.groups) ? palette.groups : []
      setPaletteGroups(groups)
      ...
      if (stripePattern.length === 0) {
        setStripeUnits([])
        setReextractError("未检测到清晰的格子结构，请优化 prompt 或更换角度更佳的格子参考图。")
        if (typeof window !== "undefined") {
          window.sessionStorage.removeItem(STRIPE_STORAGE_KEY)
        }
      } else {
      ...
      window.requestAnimationFrame(() => {
        try {
          ...
          const baseRenderConfig = {
            ...
            resolveSegment: (stripeIndex: number) => {
              const unit = stripeUnits[stripeIndex]
              const width = baseWidths[stripeIndex]
              if (!unit || !Number.isFinite(width) || width <= 0) return null
              return {
                width: Math.max(1, Math.round(width * stripeThicknessScale)),
                fillStyle: `rgba(${clampChannel(unit.color.r)}, ${clampChannel(unit.color.g)}, ${clampChannel(unit.color.b)}, 0.5)`,
              }
            },
          }

          drawStripesAtAngle(ctx, baseRenderConfig)
          drawStripesAtAngle(ctx, { ...baseRenderConfig, angleOffsetDeg: 90 })
  ...
  }, [stripeUnits, totalUnitWidth, drawStripesAtAngle, stripeThicknessScale])
          ...
          <CardContent className="space-y-6">
                <div className="space-y-3">
                  ...
                  <div className="rounded-lg border border-border/30 bg-background/60 p-4">
                    {stripeUnits.length > 0 ? (
                      <div className="relative h-24 w-full overflow-hidden rounded-md border border-border/40 shadow-inner">
                        ...
                        <div className="pointer-events-none absolute inset-0 flex flex-col opacity-35 mix-blend-multiply">
                          {stripeUnits.map((unit) => (
                            <div
                              key={`${unit.id}-perpendicular`}
                              className="flex-1"
                              style={{
                                flexGrow: Math.max(1, unit.widthPx),
                                backgroundColor: `rgb(${unit.color.r}, ${unit.color.g}, ${unit.color.b})`,
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <Label
                      htmlFor="stripe-thickness-slider"
                      className="text-xs font-medium uppercase tracking-wide text-muted-foreground"
                    >
                      条纹粗细
                    </Label>
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-foreground">×{stripeThicknessScale.toFixed(2)}</span>
                      <Button
                        type="button"
                        size="sm"
                        variant="ghost"
                        className="h-7 px-2 text-xs"
                        onClick={() => setStripeThicknessScale(1)}
                        disabled={stripeThicknessScale === 1}
                      >
                        重置
                      </Button>
                    </div>
                  ...
                  <Slider
                    id="stripe-thickness-slider"
                    min={0.5}
                    max={2}
                    step={0.05}
                    value={[stripeThicknessScale]}
                    onValueChange={(value) => {
                      const nextValue = Array.isArray(value) ? value[0] : value
                      const numericValue =
                        typeof nextValue === "number" && Number.isFinite(nextValue) ? nextValue : stripeThicknessScale
                      const clamped = Math.max(0.5, Math.min(2, numericValue))
                      setStripeThicknessScale(Number.isFinite(clamped) ? Number(clamped.toFixed(2)) : 1)
                    }}
                  />
                </div>
                <Separator />
