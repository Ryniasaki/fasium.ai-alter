...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${TENANT_API_BASE}/reports/trending/generate_baokuan`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      const errorBody =
        data && typeof data === "object" ? data : { detail: text || "Generation failed" }
    ...
    console.error("Failed to call tenant generate_baokuan:", error)
