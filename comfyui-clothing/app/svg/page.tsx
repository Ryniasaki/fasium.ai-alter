...
import { ArrowUpRight, Download, FileText, PenTool, Sparkles, Upload } from "lucide-react"
...
const SVG_PREFILL_KEY = "svg_prefill_image"

export default function SvgVectorizationPage() {
  ...
  useEffect(() => {
    const prefill = window.sessionStorage.getItem(SVG_PREFILL_KEY)
    if (!prefill) return

    window.sessionStorage.removeItem(SVG_PREFILL_KEY)
    ...
    try {
      ...
      void (async () => {
        try {
          ...
          const file = new File([blob], `svg-prefill-${Date.now()}.${ext}`, { type: blob.type || "image/png" })
          ...
          console.error("Prefill svg failed:", error)
          setError("自动载入任务图片失败，请手动上传。")
        }
      })()
    } catch (error) {
      console.error("Invalid svg prefill payload:", error)
    }
  }, [resetState])
  ...
  const startVectorization = useCallback(async () => {
    if (!selectedFile) {
      setError("请先选择一张图片")
      return
    }
    ...
    try {
      const resp = await extractApiClient.submitSvgVectorization(selectedFile)

      setStatus("processing")
      setStatusMessage("正在矢量化...")
      setProgress(30)
      ...
      for (let i = 0; i < MAX_ATTEMPTS; i++) {
        ...
        if (currentStatus.status === "FAILED") throw new Error("矢量化任务失败")
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center">
            <div className="flex-1 space-y-4">
              ...
              >
                ...
                F13 SVG Vectorization
              </Badge>
              <h1 className="text-4xl font-bold tracking-tight text-white md:text-5xl">
                SVG Vector Lab
              </h1>
              <p className="text-lg text-white/70">
                把灵感图片快速转成可编辑的 SVG 矢量文件，保留结构层级，方便在 Illustrator / CLO / Browzwear 中进一步修改。
              </p>
              <div className="flex flex-wrap gap-3 text-sm text-white/60">
                <span className="inline-flex items-center gap-2 rounded-full border border-white/10 px-3 py-1">
                  <PenTool className="size-4 text-emerald-300" />
                  轮廓智能跟踪
                </span>
                <span className="inline-flex items-center gap-2 rounded-full border border-white/10 px-3 py-1">
                  ...
                  >
                    {previewUrl ? (
                      ...
                      <div className="space-y-3">
                        ...
                        <p className="text-sm text-white/60">支持 JPG / PNG / WebP，建议带有清晰边缘</p>
                      </div>
                    )}
                  </div>
                  <input
        ...
        {outputs.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm uppercase tracking-[0.4em] text-white/50">SVG Results</p>
                <h3 className="text-2xl font-semibold text-white">矢量结果</h3>
                <p className="text-sm text-white/70">点击卡片即可在新标签中打开并下载 SVG 文件</p>
              </div>
            </div>
              ...
              {outputs.map((url, index) => (
                ...
                >
                  <div className="relative h-80">
                    <Image
                      src={url}
                      alt={`SVG result ${index + 1}`}
                      fill
                      sizes="(max-width: 768px) 100vw, 50vw"
                      className="object-contain bg-slate-900/50 p-6 transition duration-500 group-hover:scale-105"
                    />
